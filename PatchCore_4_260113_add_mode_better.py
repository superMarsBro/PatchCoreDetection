...
class ResNetFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        # 【优化点1】使用 layer2 和 layer3 (更鲁棒，对噪点不敏感)
        # 原代码是 layer1 和 layer2
        self.feature_extractor = create_feature_extractor(
            base_model, return_nodes={"layer2": "s2", "layer3": "s3"}
        )

        # 【优化点2】增大平滑核 Kernel=5 (原为3)
        # 作用：进一步模糊局部高频特征，降低正常纹理的差异
    ...
    def forward(self, x):
        ...
        # 将 s3 (小尺寸) 上采样对齐到 s2 (大尺寸)
        # 注意：这里 keys 变成了 s2 和 s3
...
class AutoDefectDetector:
    ...
    # === 核心逻辑提取：计算热力图 ===
    # 在 AutoDefectDetector 类中修改 get_heatmap 方法
    def get_heatmap(self, img_path):
        ...
        # 【优化点3】取平均距离而不是最短距离 (代码原本就是mean，这里确认一下)
        # 如果想要更平滑，可以尝试去掉最大值再平均：
        # score_map = np.mean(distances[:, 1:], axis=1).reshape(fh, fw)
        score_map = np.mean(distances, axis=1).reshape(fh, fw)

        # 【优化点4】增强高斯模糊
        # 原本是 (3,3)，现在改为 (9,9)
        # 作用：将孤立的高分噪点抹平，降低 MaxScore
...
def main():
    ...
    LIB_FILE = "PatchCoreDetection\\\\feature_lib\\\\lib_resnet18_260113_1.npz"
