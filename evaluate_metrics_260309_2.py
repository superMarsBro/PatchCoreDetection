...
def evaluate_academic_metrics(detector, good_dir, bad_dir):
    ...
    # 2. 引入 sklearn 的 precision_recall_curve 寻找全局最优阈值 (F1-Max)
    from sklearn.metrics import precision_recall_curve

    precisions, recalls, thresholds = precision_recall_curve(y_true, y_scores)

    # 计算所有可能的 F1-Score，并处理分母为 0 的情况
    f1_scores = np.divide(
    ...
    print("\n" + "🌟 学术论文评估指标结果 (Global Optimal Threshold) 🌟")
    print("-" * 50)
    print(f"   ➤ Image-level AUROC : {auroc * 100:.2f} %")
    print(f"   ➤ MaxScore on Good  : {max_good_score:.4f} (越低越好)")
    print("-" * 50)
    print(f"   【全局最佳阈值 (Best Thresh) : {best_thresh:.4f}】")
    print(f"   ➤ Precision (查准率) : {best_precision * 100:.2f} %")
    print(f"   ➤ Recall (查全/召回) : {best_recall * 100:.2f} %")
    print(f"   ➤ F1-Score (综合)    : {best_f1 * 100:.2f} %")
    print(f"   ➤ FPR (误检/假阳率)  : {best_fpr * 100:.2f} %  ({fp} 张好布被误判)")
    print("=" * 50)
...
def main():
    ...
    LIB_FILE = "feature_lib\lib_resnet18_260309_small_AvgPool_GaussianBlur.npz"
